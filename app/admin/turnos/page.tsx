...
export default function AdminTurnosPage() {
  ...
  const [editarRol, setEditarRol] = useState<{ id: string; nombre: string; rolActual: string } | null>(null);
  ...
  const { data: usuariosData, loading: loadingUsuarios, refetch: refetchUsuarios } = useFetch(
  ...
  // Filtrar usuarios excluyendo admins
  const usuariosFiltrados = useMemo(() => {
    return (usuariosData || []).filter((u: any) => u.role !== "admin");
  }, [usuariosData]);
  ...
  const handleUpdateRole = async () => {
    if (!editarRol || !nuevoRol) return;

    setEditandoRol(true);
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_USER}/${editarRol.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      alert(`Rol actualizado correctamente. ${editarRol.nombre} ahora es ${nuevoRol}`);
      setEditarRol(null);
      setNuevoRol("");
      refetchUsuarios();
    } catch (error) {
            ...
            <div className="flex justify-between items-center mb-4">
              <div>
                <h1 className="text-3xl font-extrabold text-[#536a86] tracking-tight drop-shadow mb-2">
                  {mostrarUsuarios ? "Administración de Usuarios" : "Administración de Turnos"}
                </h1>
                <p className="text-[#7a8fa6] text-lg">
                  {mostrarUsuarios 
                    ? "Gestiona y administra todos los usuarios del spa" 
              ...
              <Button
                ...
                className="bg-[#536a86] hover:bg-[#435c74] text-white transition-all duration-300 shadow-lg hover:shadow-xl"
              >
                ...
                {mostrarUsuarios ? "Ver Turnos" : "Ver Usuarios"}
              </Button>
            </div>
          </div>

          {/* Filtros - Solo mostrar cuando estamos en vista de turnos */}
          {!mostrarUsuarios && (
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="bg-[#536a86] hover:bg-[#435c74] transition-colors duration-300">
                        ...
                        <TableHead className="text-white font-semibold">Apellido</TableHead>
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
              <div className="bg-white rounded-lg p-6 w-full max-w-md mx-4">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-[#536a86]">
                ...
                <div className="mb-4">
                  <p className="text-sm text-[#7a8fa6] mb-2">
                    Usuario: <span className="font-medium text-[#536a86]">{editarRol.nombre}</span>
                  </p>
                  <p className="text-sm text-[#7a8fa6] mb-4">
                    Rol actual: <span className="font-medium text-[#536a86]">{editarRol.rolActual}</span>
                  </p>
                  ...
                  <select
                    className="w-full border border-[#c3d0e6] px-3 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#536a86] focus:border-transparent transition-all duration-300 bg-white"
                    value={nuevoRol}
                ...
                <div className="flex gap-3 justify-end">
                  ...
                  <Button
                    ...
                    disabled={editandoRol || nuevoRol === editarRol.rolActual}
                    className="bg-[#536a86] hover:bg-[#435c74] text-white"
                  >
                    {editandoRol ? "Actualizando..." : "Actualizar Rol"}
                  </Button>
                </div>
              </div>
            </div>
